...
//#define APP_TIMER_MAX_TIMERS            (2 + BSP_APP_TIMERS_NUMBER) // NOT USED?  /**< Maximum number of simultaneously created timers. */
...
void write_output_pulse(uint8_t pulse_width);
...
//		nrf_gpio_cfg( OUTPUT_PIN, NRF_GPIO_PORT_DIR_OUTPUT, NRF_GPIO_PIN_INPUT_DISCONNECT, 
//			NRF_GPIO_PIN_PULLDOWN, NRF_GPIO_PIN_S0S1, NRF_GPIO_PIN_NOSENSE );	
	nrf_gpio_pin_write(OUTPUT_PIN, 0);
	
}

#define HF_PULSE_WIDTH    12 // ms
void write_output_pulse(uint8_t pulse_width)
{
	uint8_t num_rep = 0;
	uint8_t i=0;
  uint8_t	val=1;
	num_rep = pulse_width / HF_PULSE_WIDTH;
	printf("1...%d,%d", pulse_width, num_rep);
	for (i=0; i<num_rep; ++i)
	{
		nrf_gpio_pin_write(OUTPUT_PIN, val);
		val ^= 1;
		nrf_delay_ms(HF_PULSE_WIDTH);
	}
	nrf_gpio_pin_write(OUTPUT_PIN, 0);
//	nrf_gpio_pin_write(OUTPUT_PIN, 1);
//	nrf_delay_ms(pulse_width);
//	nrf_gpio_pin_write(OUTPUT_PIN, 0);
	printf("...0\r\n");
}
